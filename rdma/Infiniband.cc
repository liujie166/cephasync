...
char *Infiniband::MemoryManager::PoolAllocator::malloc(const size_type bytes)
{
  ...
  m = static_cast<mem_info *>(manager->malloc(nbufs * sizeof(Chunk)+sizeof(*m)));
  if (!m) {
    lderr(cct) << __func__ << " failed to allocate " << sizeof(Chunk) << " * " << nbufs
    << " bytes for nbufs*sizeof(chunk) + " << sizeof(*m) << " bytes for mem_info"<< dendl;
    return NULL;
  }
  ...
  for (unsigned i = 0; i < nbufs; i++) {
    ...
    ch->mr = ibv_reg_mr(manager->pd->pd, (char*)reinterpret_cast<uintptr_t>(ch->buffer), ch->bytes, IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_LOCAL_WRITE);
    if (ch->mr == NULL) {
